...
#
# Usage:
#   ./scripts/dev-caddy.sh                    # local backend + frontend
#   ./scripts/dev-caddy.sh --remote <url>     # local frontend, remote backend
set -euo pipefail
...
check_cmd() {
  if ! command -v "$1" &>/dev/null; then
    ...
    error "Run ./scripts/dev-caddy-setup.sh first, or install $1 manually."
    exit 1
  fi
}
...
if [[ "$RESOLVED" != "127.0.0.1" ]]; then
  ...
  error "Run ./scripts/dev-caddy-setup.sh to configure dnsmasq."
  exit 1
fi
...
PROJECT_ROOT="$ROOT" caddy run --config "$ROOT/scripts/dev/Caddyfile" >"$DEV_DIR/caddy.log" 2>&1 &
CADDY_PID=$!
