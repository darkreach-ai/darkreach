...
#[derive(Deserialize)]
pub(super) struct DocSearchQuery {
    q: String,
}
...
pub(super) async fn handler_api_docs_search(
...
pub(super) async fn handler_api_doc_roadmap(AxumPath(slug): AxumPath<String>) -> impl IntoResponse {
    ...
    match std::fs::read_to_string(&path) {
        Ok(content) => {
            let title = extract_title(&content, &slug);
            Json(serde_json::json!({"slug": format!("roadmaps/{}", slug), "title": title, "content": content, "category": "roadmaps"})).into_response()
        }
        Err(_) => (
        ...
        if name == slug {
            ...
            return match std::fs::read_to_string(path) {
                Ok(content) => {
                    ...
                    Json(serde_json::json!({"slug": format!("agent/{}", name), "title": title, "content": content, "category": "agent"})).into_response()
                }
                Err(_) => (
                    StatusCode::NOT_FOUND,
...
pub(super) async fn handler_api_doc(AxumPath(slug): AxumPath<String>) -> impl IntoResponse {
    ...
    match std::fs::read_to_string(&path) {
        Ok(content) => {
            let title = extract_title(&content, &slug);
            Json(serde_json::json!({"slug": slug, "title": title, "content": content}))
                .into_response()
        }
        Err(_) => (
            StatusCode::NOT_FOUND,
