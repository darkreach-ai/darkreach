...
**Repo:** `github.com/darkreach-ai/darkreach` | **Registry:** `ghcr.io/darkreach-ai/darkreach`
...
Secrets are encrypted in-repo using SOPS + age. See `secrets/README.md` for details.

- Encrypted files: `secrets/*.enc.yaml` (committed to git)
- Local `.env` files are generated by `./scripts/decrypt-secrets.sh` (never committed)
- Each developer has an age key pair; public keys listed in `.sops.yaml`
- CI uses a separate `SOPS_AGE_KEY` GitHub org secret

## Git Workflow

- **Branch protection** on `master`: require PR, 1 approval, CI passing
- **Default merge**: squash and merge (clean history)
- **Branch naming**: `feat/`, `fix/`, `chore/`, `docs/`, `deploy/`
- **Self-merge**: trivial changes (typos, deps) after 30 min with `trivial` label; engine/DB/deploy always need review
- **Deploy**: only from `master`, announce in chat first
